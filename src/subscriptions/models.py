...
class Subscription(models.Model): #Subscription Plans
    """_summary_
...
class SubscriptionPrice(models.Model):
    ...
    def __str__(self):
        return f'{self.subscription.name} {self.interval}ly {self.updated}'
    ...
    def display_sub_name(self):
        if not self.subscription:
            return 'Plan'
        return str(self.subscription.name)
    ...
    def save(self, *args, **kwargs):
        if (not self.stripe_id and 
            ...
            self.stripe_id = stripe_id
        super().save(*args, **kwargs)
        if self.featured and self.product_stripe_id:
...
def user_sub_post_save(sender, instance, *args, **kwargs):
    ...
    if subscription_obj is not None:
        ...
        groups_ids = groups.values_list('id', flat=True) # All the groups in the user's plan 
    if not ALLOW_CUSTOM_GROUPS:
        ...
        subs_groups_set = set(subs_groups) # All the groups available to all Subscription Plans
        groups_ids_set = set(groups_ids)
        current_groups = user.groups.all().values_list('id', flat=True) # All the current groups the user is in
        current_groups_ids_set = set(current_groups) - subs_groups_set
        final_group_ids = list(groups_ids_set | current_groups_ids_set)
